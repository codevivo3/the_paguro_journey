...
export default defineType({
  ...
  fields: [
    ...
    // -----------------------------------------------------------------------
    // Optional "hero" config (curation helpers)
    // -----------------------------------------------------------------------
    defineField({
      name: 'hero',
      ...
      type: 'object',
      description: biDesc(
        'Optional. Control whether this media can be used in hero slides and how it behaves on desktop vs mobile.',
        'Opzionale. Controlla se questo media può essere usato negli hero e come si comporta su desktop vs mobile.',
      ),
      fields: [
        defineField({
          name: 'enabled',
          ...
          initialValue: false,
          description: biDesc(
            'Turn on to make this media eligible for hero usage (homepage/section headers).',
            'Attiva per rendere questo media candidabile per l’hero (homepage/intestazioni sezioni).',
          ),
        }),

        defineField({
          name: 'desktopEligible',
          title: 'Desktop eligible (landscape)',
          type: 'boolean',
          initialValue: true,
          hidden: ({ parent }) => !(parent as UnknownRecord | null)?.enabled,
          description: biDesc(
            'Use this media in the DESKTOP hero (typically landscape).',
            'Usa questo media nell’hero DESKTOP (di solito landscape).',
          ),
        }),

        defineField({
          name: 'mobileEligible',
          title: 'Mobile eligible (portrait)',
          type: 'boolean',
          initialValue: true,
          hidden: ({ parent }) => !(parent as UnknownRecord | null)?.enabled,
          description: biDesc(
            'Use this media in the MOBILE hero (typically portrait).',
            'Usa questo media nell’hero MOBILE (di solito portrait).',
          ),
        }),

        defineField({
          name: 'desktopRank',
          title: 'Desktop rank (optional)',
          type: 'number',
          hidden: ({ parent }) => !(parent as UnknownRecord | null)?.enabled,
          description: biDesc(
            'Optional. Suggested ordering for desktop hero. Lower number = higher priority. (Can be ignored if ordering is curated elsewhere.)',
            'Opzionale. Ordine suggerito per l’hero desktop. Numero più basso = priorità più alta. (Può essere ignorato se l’ordine è curato altrove.)',
          ),
        ...
        defineField({
          name: 'mobileRank',
          title: 'Mobile rank (optional)',
          type: 'number',
          hidden: ({ parent }) => !(parent as UnknownRecord | null)?.enabled,
          description: biDesc(
            'Optional. Suggested ordering for mobile hero. Lower number = higher priority. (Can be ignored if ordering is curated elsewhere.)',
            'Opzionale. Ordine suggerito per l’hero mobile. Numero più basso = priorità più alta. (Può essere ignorato se l’ordine è curato altrove.)',
          ),
          validation: (r) => r.integer().min(0),
        }),
        ...
        defineField({
          ...
          hidden: ({ parent }) => !(parent as UnknownRecord | null)?.enabled,
          description: biDesc(
            'Optional. Internal note for editors (not shown on the website).',
            'Opzionale. Nota interna per editor (non mostrata sul sito).',
          ),
        }),
      ],
    }),
